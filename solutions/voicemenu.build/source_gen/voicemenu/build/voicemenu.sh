...
if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/voicemenu2021.2/voicemenu.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/voicemenu2021.2/voicemenu.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
if [ -n "$IDEA_VM_OPTIONS" ] && [ -r "$IDEA_VM_OPTIONS" ]; then
...
elif [ -r "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/voicemenu2021.2/voicemenu$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/voicemenu2021.2/voicemenu$BITS.vmoptions"
  ...
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/voicemenu$BITS.vmoptions"
fi
...
IDEA_PATHS_SELECTOR=voicemenu2021.2
"$JAVA_BIN" \
  ${AGENT} \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
