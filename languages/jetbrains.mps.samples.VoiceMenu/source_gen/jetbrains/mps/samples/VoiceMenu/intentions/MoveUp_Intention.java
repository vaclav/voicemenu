...
public final class MoveUp_Intention extends AbstractIntentionDescriptor implements IntentionFactory {
  ...
  private boolean isApplicableToNode(final SNode node, final EditorContext editorContext) {
    if (SNodeOperations.isInstanceOf(SNodeOperations.getParent(node), CONCEPTS.Menu$bP)) {
      return !(Objects.equals(node, ListSequence.fromList(SLinkOperations.getChildren(SNodeOperations.cast(SNodeOperations.getParent(node), CONCEPTS.Menu$bP), LINKS.events$AIPs)).first()));
    }
    return false;
  }
  @Override
