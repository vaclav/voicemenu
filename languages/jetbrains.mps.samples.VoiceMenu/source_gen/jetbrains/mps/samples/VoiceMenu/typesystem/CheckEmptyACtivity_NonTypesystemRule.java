...
public class CheckEmptyACtivity_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode activity, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if ((Sequence.fromIterable(SNodeOperations.ofConcept(SNodeOperations.getChildren(activity), CONCEPTS.Action$AN)).isEmpty() && Sequence.fromIterable(SNodeOperations.ofConcept(SNodeOperations.getChildren(activity), CONCEPTS.Menu$By)).isEmpty()) || (SLinkOperations.getTarget(activity, LINKS.event$pmgi) == null)) {
      {
