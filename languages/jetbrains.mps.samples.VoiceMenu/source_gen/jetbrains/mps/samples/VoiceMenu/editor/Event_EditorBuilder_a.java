...
/*package*/ class Event_EditorBuilder_a extends AbstractEditorBuilder {
  ...
  private boolean nodeCondition_ymj9mh_a4a0() {
    ...
    if (isEmptyString(SPropertyOperations.getString(myNode, PROPS.trigger$zpYt))) {
      return true;
    }
    if (isNotEmptyString(SPropertyOperations.getString(myNode, PROPS.trigger$zpYt)) && isEmptyString(SPropertyOperations.getString(myNode, PROPS.name$tAp1))) {
      return false;
    }
    boolean b = Sequence.fromIterable(SNodeOperations.ofConcept(SNodeOperations.getChildren(SNodeOperations.getParent(myNode)), CONCEPTS.Activity$oQ)).any(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return SPropertyOperations.getString(SLinkOperations.getTarget(it, LINKS.event$gjCV), PROPS.trigger$zpYt) == SPropertyOperations.getString(myNode, PROPS.trigger$zpYt);
      }
    });
    if (!(b)) {
      return true;
